...
exports.uploadFile = async function (req, res, next) {
    ...
    if (!fileInfo) {
        ...
        return res.status(400).render('error-page', { message: "No file data received." });
    }
    ...
    if (fileInfo.buffer) {
        ...
        try {
            ...
            return res.status(500).render('error-page', { message: "Failed to read the uploaded file from disk." });
        }
    } else {
        console.error('Multer did not provide a file buffer or path.')
        return res.status(500).render('error-page', { message: "Internal server error: File data not accessible." });
    }
    ...
    try {
        ...
        if (supabaseUploadError) {
            ...
            return res.status(500).render('error-page', { message: "File upload to Supabase failed." });
        }
...
exports.getFileDetails = async (req, res, next) => {
    ...
    try {
        ...
        if (!file) {
            return res.status(404).render('error-page', { message: "File not found." });
        }
